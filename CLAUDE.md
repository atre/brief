...
- `src/cli.ts` — hand-rolled args (`--brief` sets top 3, `--gates` repo-only, `--write` hub-only, `--section a,b` radar/repo `--json` only, `--missing` feedback-only) + HELP text.
- `src/types.ts` — `Repo`, `Report`, `DocInfo`, `FeedbackInfo`, `SessionInfo`, `GitInfo`, `PlanInfo` shapes.
...
- `src/runtime.ts` — pulse last snapshot (`~/.pulse/snaps/last.json`) → `runtimeFor(service, findings)`; bare ids match any kind, `node:*`-style ids one kind; `service` is comma-separated, trailing `*` = prefix match (cron families); worst severity across all matches wins.
...
- `src/collect.ts` — assembles `Repo` (8-way concurrency); CLAUDE.md checkbox fallback for `next`; gates summary (`passed/total`, skipped excluded); `tagSharedDescriptions` (post-pass over the full collected set — byte-identical `description` across ≥2 repos → `+1` reason tag, both directions).
...
- `src/render.ts` — radar text (2–3 lines per repo, fold + quiet lines; `--brief` ≤ 15 lines ending with `… +N more`), handoff (`--tokens` budget → list sizes; `gates: ✓ 3/3`; `plans: N open (file d/total, …) · M done`), `--md`, hub / queue / diff / lessons renders, `renderFeedback` (`--missing` mode + default-view footer), `selectSections`/`selectReport` (`--json --section` filtering).
